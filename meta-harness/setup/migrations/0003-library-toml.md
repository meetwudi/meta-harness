...
- local Memory Library locations such as `library://task-memory`
...
- `meta-harness/templates/task-memory/LIBRARY.toml`
...
3. If the Library is the repository root, add `LIBRARY.toml` with no task update
   access unless the human approves another policy.
4. If the Library is a shared harness Library such as `library://meta-harness`
   or `library://project-harness`, add `LIBRARY.toml` with no task update
   access unless the human approves another policy.
5. If the Library is local task memory, add `LIBRARY.toml` with
   task-URI-pattern update access for the tasks that own that memory.
