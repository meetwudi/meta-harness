	...
	async generateSuggestions(suggestionId, config, consumerAgentId) {
		...
		try {
			...
			await agent.runAgent({
				context: this.core.getContextForAgent(consumerAgentId),
				forwardedProps: {
...
var RunHandler = class {
	...
	async connectAgent({ agent }) {
		try {
			...
			if (isFreshRestore) {
				...
				agent.setState({});
				const proxied = agent;
				if (incomingThreadId && typeof proxied.clearReplayCursor === "function") proxied.clearReplayCursor(incomingThreadId);
			}
	...
	* Run an agent
	*/
	async runAgent({ agent, forwardedProps, resume }) {
