...
const ChoicePicker = createLitComponent(ChoicePickerApi, ({ props, context, state, requestUpdate }) => {
	const local = state;
	...
	return html`
      ...
      >
        ${props.label ? html`<strong style="font-size: 14px;">${props.label}</strong>` : nothing}
        ${props.filterable ? html`<input
              type="text"
		...
		if (props.displayStyle === "chips") return html`
                <button
                  type="button"
...
//#endregion
export { ChoicePicker };
//# sourceMappingURL=choice-picker.mjs.map